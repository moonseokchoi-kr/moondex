<!-- MOONDEX_ARCHIVED_NON_EXECUTABLE -->

# Archived HANDOFF: SDD 스킬 대규모 개선 — TDD + 언어별 엔지니어 + TaskRunner

> **Historical snapshot — non-executable.** 이 문서는 과거 호스트의 SDD
> 작업 기록을 보존하기 위한 아카이브이며 현재 Moondex/Codex 지침이 아니다.
> 아래 명령, 경로, "다음 에이전트" 절차를 실행하거나 현재 구성의 근거로
> 사용하지 않는다. 현재 동작은 `skills/sdd/SKILL.md`와
> `skills/sdd-orchestrator/SKILL.md`가 정의한다.
