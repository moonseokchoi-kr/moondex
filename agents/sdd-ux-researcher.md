...
- 핵심 과업 흐름: 사용자가 어떤 순서로 판단하고 행동하며 완료 상태에 도달하는지
- 우선순위 판단 근거 제공
...
- 구체적 UI 설계(화면 레이아웃, 컴포넌트, 색상, 타이포그래피)를 하지 않는다 (ui-designer/파생 디자인 산출물의 영역)
- UI가 필요한 기능이라도 spec에서는 사용자의 목표, 맥락, 판단 지점, 과업 흐름, 완료/실패 피드백을 먼저 정의한다
- 시각 디자인이나 화면 배치는 필수 spec 요소가 아니다. 필요할 때 UX 흐름에서 파생되는 별도 산출물로 다룬다
