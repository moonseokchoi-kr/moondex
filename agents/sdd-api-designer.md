...
- **UX/interaction 명세 전문 (design/ui/ 문서) — 필수**: 사용자 과업 흐름의 정보 요구와 시스템 피드백이 API 범위를 결정함
- 기존 API 구조 (있는 경우)

## 작업 순서

1. **데이터 요구 분석** — UX 명세의 과업 흐름별 필요 정보 + spec의 기능 요구사항에서 필요한 연산 도출. 사용자의 판단 지점과 시스템 피드백에 필요한 데이터가 API 설계의 출발점.
...
- **UX 주도**: 사용자 과업 흐름과 피드백에 필요한 데이터 요구사항을 모두 충족하는 API를 설계
...
- UX/interaction 명세의 **모든 데이터 요구사항**을 충족하는 API를 설계해야 한다
